...
NavPolygon * Pathfinder::LessCost(const std::vector<NavPolygon*>& vector){
	...
	int heuristicCost;
	int minorHeuristicCost = 100000000;
	//Heuristica: distancia del polígono al punto final
	if (vector.size() > 1) { //Para evitar el caso de la primera iteracion que el poligono no tiene padre
		for (int i = 0; i < vector.size(); i++) {
			//Calculamos porque lado hemos llegado a ese polígono
			for (int j = 0; j < vector[i]->m_Edges.size(); j++) {
				if ((vector[i]->father)->id == vector[i]->m_Edges[j].m_idNeighbour) {
					USVec2D halfpoint = HalfPoint(vector[i]->m_verts[vector[i]->m_Edges[j].m_verts[0]], vector[i]->m_verts[vector[i]->m_Edges[j].m_verts[1]]);
					heuristicCost = (halfpoint - m_EndPosition).Length();
				}
			}
